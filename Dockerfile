...
RUN apt-get update && apt-get install -y git ca-certificates && update-ca-certificates --fresh && mkdir /cwl && chown -R airflow:airflow /cwl && chmod -R a+rwx /cwl && chmod a+rx /usr/bin/install_cwl_airflow && chmod a+rwx /cwl-airflow.zip
